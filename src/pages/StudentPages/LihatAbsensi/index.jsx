...
function LihatAbsensi() {
  ...
  const { data: dataStudents, loading: loadingGetUser } = useQuery(
    GET_ONE_USER,
    {
      variables: {
  ...
  const { data, loading: loadingGetSchedule } = useSubscription(
    GET_SCHEDULE_BY_STUDENT,
    {
      variables: {
        npm,
      },
      ...
      <div className="w-full bg-primary-white2 dark:bg-primary-black lg:px-7 pt-20 lg:text-xl text-xs h-screen">
        <div className="flex lg:flex-row flex-col justify-center items-center p-3">
          ...
          <div className="flex lg:flex-row flex-col justify-center items-center gap-2">
            <h1 className="lg:text-2xl lg:text-left text-lg text-center text-primary-grey dark:text-white">
              Attendance
            </h1>
            <h1 className="lg:text-2xl lg:text-left text-lg text-center text-primary-grey dark:text-white opacity-50">
              {" "}
              {AUTH.getFullname()}
            </h1>
          </div>
          ...
          <div className="flex lg:flex-row md:flex-row flex-col gap-x-10 gap-y-5 flex-wrap justify-center items-center py-3">
            {loadingGetUser || loadingGetSchedule ? (
              ...
              data?.schedules.map((d) => (
                <div
                  key={d.id}
                  className="p-6 lg:w-1/3 md:w-1/3 w-3/4 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700"
                >
                  <h5 className="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                    {d.class.class_name} - {d.course.course_name}
                  </h5>
                  <p className="mb-3 text-sm text-gray-700 dark:text-gray-400">
                    Teacher - {d.lecturer.fullname}
                  </p>
                  <p className="mb-3 text-sm text-gray-700 dark:text-gray-400">
                    {d.room} , {d.day} / {d.time}
                  </p>
