...
function ModalDelete({ data, type }) {
  ...
  const [lecturerDelete, setLecturerDelete] = useState({
    nidn: "",
    fullname: "",
  });
  const [classNameDelete, setClassNameDelete] = useState({
    id: "",
    class_name: "",
  });
  ...
  const [deleteStudent, { loading: loadingDeleteStudent }] = useMutation(
    DELETE_STUDENT,
    {
      onCompleted: () => {
        Swal.fire({
  ...
  const [deleteLecturer, { loading: loadingDeleteLecturer }] = useMutation(
    DELETE_LECTURER,
    {
      onCompleted: () => {
        Swal.fire({
  ...
  const [deleteClassName, { loading: loadingDeleteClassName }] = useMutation(
    DELETE_CLASS_NAME,
    {
      onCompleted: () => {
        Swal.fire({
  ...
  const [deleteCourse, { loading: loadingDeleteCourse }] = useMutation(
    DELETE_COURSE,
    {
      onCompleted: () => {
        Swal.fire({
    ...
    DELETE_SCHEDULE,
    {
      onCompleted: () => {
        Swal.fire({
          position: "top-end",
          icon: "success",
          title: "Schedule & Attendance Removed Successfully",
          showConfirmButton: false,
          timer: 1200,
        });
    ...
    <>
      ...
      {showModal && (
        <div
          id="modal-delete"
          tabIndex="-1"
          className="flex items-center overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full"
        >
              ...
              <div className="p-6 text-center">
                ...
                {type === "student" ? (
                  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <b>{studentDelete.fullname}</b>
                  </h3>
                ) : type === "lecturer" ? (
                  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                    It will also Delete Attendance of Lecturer <br />{" "}
                    <b>{lecturerDelete.fullname}</b>
                  </h3>
                ) : type === "class" ? (
                  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                    It will also Delete Attendance for the Class <br />{" "}
                    <b>{classNameDelete.class_name}</b>
                  </h3>
                ) : type === "course" ? (
                  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                    It will also Delete Attendance of a Course <br />{" "}
                    <b>{courseDelete.course_name}</b>
                  </h3>
                ) : (
                  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                    Are you sure you want to delete Schedule & Attendance for
                    Class <br />{" "}
                    <b>
                ...
                >
                  ...
                  loadingDeleteLecturer ||
                  loadingDeleteClassName ||
                  loadingDeleteCourse ||
