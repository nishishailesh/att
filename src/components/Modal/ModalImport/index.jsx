...
function ModalImport() {
  ...
  const [importStudents, { loading: loadingImport }] = useMutation(
    IMPORT_STUDENTS,
    {
      onCompleted: () => {
        Swal.fire({
    ...
    promise.then((d) => {
      d.forEach((data) => {
        ...
        setUsers((users) => [
          ...
          {
            fullname: data.fullname,
            username: data.npm.toString(),
            password: data.npm.toString(),
            study_programs_id: id_prodi,
            roles_id: 4,
          },
        ]);
    ...
    <>
      ...
      {modalImport && (
        <div
          id="modalImport"
          tabIndex="-1"
          className="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full shadow-lg"
        >
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-6 space-y-6">
                <div className="text-sm flex flex-col gap-y-2 dark:text-white text-primary-grey">
                  <label htmlFor="">Import Student Data</label>
                  <input
                    type="file"
                    onChange={(e) => {
