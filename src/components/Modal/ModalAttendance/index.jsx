...
import { INSERT_TIMESTAMP_RECORD } from "../../../api/Model/Mutation/Insert/InsertTimestampRecord";
...
function ModalAttendance({ data, role }) {
  ...
  const meet_number = useSelector((state) => state.filter.meet_number);

  const [timestamp, setTimestamp] = useState([]);
  const { data: dataTimestamp, loading } = useSubscription(GET_TIMESTAMP_RECORD, {
    variables: {
  ...
  useEffect(() => {
    setTimestamp(dataTimestamp?.timestamp_record);
  }, [dataTimestamp]);

  const [confirmCloseAttendance, setConfirmCloseAttendance] = useState(false);

  const [insertTimestampRecord] = useMutation(INSERT_TIMESTAMP_RECORD, {
    variables: {
      schedules_id: data.id,
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div className="flex justify-between items-center p-5 rounded-t border-b dark:border-gray-600">
                <h3 className="text-xl font-medium text-gray-900 dark:text-white">
                  Attendance {assignValue.course_name} - {assignValue.class_name}
                </h3>
              ...
              {data.meet_number !== 61 ? (
                <>
                  ...
                  {role !== "lecturer"
                    ? !loading &&
                      (timestamp.filter((t) => t.meet_number === parseInt(meet_number)).length !== 0 ? (
                        timestamp
                          ?.filter((t) => t.meet_number === parseInt(meet_number))
                          .map((date) => (
                            <div key={date.id} className="flex px-8 py-2 justify-start text-primary-grey dark:text-white">
                              Held On : {date.date_meet}
                            </div>
                          ))
                      ) : (
                        <div className="flex px-8 py-2 justify-start text-red-600">Class hasn't started yet</div>
                      ))
                    : []}
                ...
                <p className="mb-3 text-sm text-gray-700 dark:text-gray-400">Session Over</p>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="p-6 text-center">
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                  {date === "" ? "PLEASE INSERT DATE!" : `Are you sure you want to complete the Attendance Session ${data.meet_number + 1} ?`}
                  <br />
                </h3>
                {date !== "" && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setShowModal(false);
                      data.meet_number !== 60 && updateMeetNumber();
                    }}
                  >
                    {loadingMeetNumber ? <LoadingAnimation /> : "Done"}
                  </button>
                )}
                <button
                  type="button"
